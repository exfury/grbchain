...
Iavl=v0.14.3
Name=grbchain
ServerName=grbchaind
ClientName=grbchaincli
...
ldflags = -X $(GithubTop)/exfury/grbchain/libs/cosmos-sdk/version.Version=$(Version) \
	-X $(GithubTop)/exfury/grbchain/libs/cosmos-sdk/version.Name=$(Name) \
  -X $(GithubTop)/exfury/grbchain/libs/cosmos-sdk/version.ServerName=$(ServerName) \
  -X $(GithubTop)/exfury/grbchain/libs/cosmos-sdk/version.ClientName=$(ClientName) \
  -X $(GithubTop)/exfury/grbchain/libs/cosmos-sdk/version.Commit=$(COMMIT) \
  -X $(GithubTop)/exfury/grbchain/libs/cosmos-sdk/version.CosmosSDK=$(CosmosSDK) \
  -X $(GithubTop)/exfury/grbchain/libs/cosmos-sdk/version.Tendermint=$(Tendermint) \
  -X "$(GithubTop)/exfury/grbchain/libs/cosmos-sdk/version.BuildTags=$(build_tags)" \

ifeq ($(WITH_ROCKSDB),true)
  ldflags += -X github.com/exfury/grbchain/libs/tendermint/types.DBBackend=rocksdb
endif

ifeq ($(MAKECMDGOALS),testnet)
  ldflags += -X github.com/exfury/grbchain/libs/cosmos-sdk/server.ChainID=okbchaintest-195
endif
...
grbchain: check_version
	$(cgo_flags) go install -v $(BUILD_FLAGS) -tags "$(build_tags)" ./cmd/grbchaind
	$(cgo_flags) go install -v $(BUILD_FLAGS) -tags "$(build_tags)" ./cmd/grbchaincli
...
cli:
	go install -v $(BUILD_FLAGS) -tags "$(build_tags)" ./cmd/grbchaincli

server:
	go install -v $(BUILD_FLAGS) -tags "$(build_tags)" ./cmd/grbchaind
...
build:
ifeq ($(OS),Windows_NT)
	go build $(BUILD_FLAGS) -tags "$(build_tags)" -o build/grbchaind.exe ./cmd/okbchaind
	go build $(BUILD_FLAGS) -tags "$(build_tags)" -o build/grbchaincli.exe ./cmd/okbchaincli
else
	go build $(BUILD_FLAGS) -tags "$(build_tags)" -o build/grbchaind ./cmd/okbchaind
	go build $(BUILD_FLAGS) -tags "$(build_tags)" -o build/grbchaincli ./cmd/okbchaincli
endif
...
build-docker-grbchainnode:
	$(MAKE) -C networks/local

# Run a 4-node testnet locally
localnet-start: localnet-stop
	@if ! [ -f build/node0/grbchaind/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/okbchaind:Z okbchain/node testnet --v 4 -o . --starting-ip-address 192.168.10.2 --keyring-backend=test ; fi
	docker-compose up -d
