...
download() {
  rm -rf "$HOME"/.grbchain/src
  mkdir -p "$HOME"/.grbchain/src
  tag=`wget -qO- -t1 -T2 --no-check-certificate "https://api.github.com/repos/exfury/grbchain/releases/latest" | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g'`
  wget --no-check-certificate "https://github.com/exfury/grbchain/archive/refs/tags/${tag}.tar.gz" -O "$HOME"/.okbchain/src/okbchain.tar.gz
  ver=$(echo $tag| sed 's/v//g')
  cd "$HOME"/.grbchain/src && tar zxvf okbchain.tar.gz &&  cd okbchain-"$ver"
}
...
Installgrbchain() {
  echo "Installgrbchain...."

  download
  cd "$HOME"/.grbchain/src/okbchain-${ver}
  checkjcmalloc
  #if alpine add LINK_STATICALLY=true
  echo "compile grbchain...."
  ...
  if ! type grbchaind > /dev/null 2>&1; then
    export PATH=$PATH:$HOME/go/bin
  fi
  echo "Installgrbchain completed"
}
