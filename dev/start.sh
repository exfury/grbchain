...
#    grbchaind start --pruning=nothing --rpc.unsafe \
    nohup grbchaind start --rpc.unsafe \
...
grbchaincli tx gov submit-proposal upgrade $CURDIR/proposals/wasm.proposal --from captain --fees 1okb  -y -b block
grbchaincli tx gov vote 1 yes --from captain --fees 0.01okb  -y -b block
