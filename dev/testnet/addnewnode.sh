...
if [ -d ${OKCHAIN_NET_CACHE}/node0/grbchaind ]; then
    seed_addr=$(${BIN_NAME} tendermint show-node-id --home ${OKCHAIN_NET_CACHE}/node0/grbchaind)@${IP}:${seedp2pport}
fi
...
init() {
    ...
    ${BIN_NAME} init ${NAME} -o --chain-id ${CHAIN_ID} --home ${OKCHAIN_NET_CACHE}/${NAME}/grbchaind --node-index ${INPUT_INDEX}
}
...
start() {
    ...
    rm ${OKCHAIN_NET_CACHE}/${NAME}/grbchaind/config/genesis.json
    cp ${OKCHAIN_NET_CACHE}/node0/grbchaind/config/genesis.json ${OKCHAIN_NET_CACHE}/${NAME}/okbchaind/config/
...
#     echo "${BIN_NAME} --home ${OKCHAIN_NET_CACHE}/${NAME}/grbchaind  start --p2p.laddr tcp://${IP}:${p2pport} --p2p.seeds ${seednode} --rpc.laddr tcp://${IP}:${rpcport}"
...
#    LOG_LEVEL=main:info,*:error,state:debug,consensus:debug
    ...
    ${BIN_NAME} start \
    --chain-id ${CHAIN_ID} \
    --home ${OKCHAIN_NET_CACHE}/${NAME}/grbchaind \
