...
function get_latest_height() {
  ...
  height=`grbchaincli status --node http://${IP}:${port} | jq .sync_info.latest_block_height | awk '{ gsub(/"/,""); print $0 }'`
  echo $height
}
...
function start_node() {
  ...
  nohup ${BIN_NAME} start \
    --chain-id ${CHAIN_ID} \
    --home cache/node${index}/grbchaind \
...
function add_val() {
  index=$1
  node_name=val${index}
  seed_addr=$(grbchaind tendermint show-node-id --home cache/node0/okbchaind)@${IP}:${seedp2pport}
  echo "add val >>> "$node_name

  grbchaind_opts="--p2p.allow_duplicate_ip  --p2p.pex=false  --p2p.addr_book_strict=false  --consensus.timeout_commit 600ms    --upload-delta=false  --elapsed DeliverTxs=0,Round=1,CommitRound=1,Produce=1  --consensus-role=v${index}  --p2p.seeds ${seed_addr} "

  start_node $index $node_name $grbchaind_opts
}
...
function add_seed() {
  ...
  grbchaind_opts="--p2p.seed_mode=true  --p2p.allow_duplicate_ip  --p2p.pex=false  --p2p.addr_book_strict=false  --consensus.timeout_commit 600ms  --upload-delta=false  --elapsed DeliverTxs=0,Round=1,CommitRound=1,Produce=1  --consensus-role=v$index "
...
function add_rpc() {
  ...
  grbchaind_opts="--p2p.seeds ${seed_addr} "
  start_node $index $node_name $grbchaind_opts
}
...
function case_1() {
  ...
  #STEP kill rpc
  killbyname_gracefully "cache/node4/grbchaind"
  sleep 2

  #STEP BUILD version2
  build_grbchain $version2
  ...
  #STEP kill 25% v
  killbyname_gracefully "cache/node3/grbchaind"
  sleep 3
...
function case_2() {
  ...
  #STEP upgrade 100% v
  killbyname_gracefully "cache/node2/grbchaind"
  sleep 3
  add_val 2

  killbyname_gracefully "cache/node1/grbchaind"
  sleep 3
  add_val 1

  killbyname_gracefully "cache/node0/grbchaind"
  sleep 3
  add_seed
  ...
  #STEP kill rpc
  killbyname_gracefully "cache/node4/grbchaind"
  killbyname_gracefully "cache/node5/grbchaind"
  sleep 3
...
### send two params , the first is the old version of grbchain, the second is the newer version.
exc_version1=$1
exc_version2=$2
