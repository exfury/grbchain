...
check_go_verison() {
  ...
  if version_lt $go_version $GO_VERSION; then
     echo "ERROR: grbchain requires go${GO_VERSION}+,please install"
     exit 1
  fi
...
check_rocksdb_version() {
  ...
  *)
  ...
  if [ "$rocksdb_version" != "$ROCKSDB_VERSION" ]; then
    echo "ERROR: grbchain requires rocksdb-v${ROCKSDB_VERSION},current: v$rocksdb_version , please install with command (make rocksdb)"
    exit 1
  fi
