...
import (
	...
	db "github.com/exfury/grbchain/libs/tm-db"
...
func TestParseMsgWrapper(t *testing.T) {
	testcases := []struct {
		...
		{
			input: `{"type": "grbchain/staking/MsgDeposit","value": {"delegator_address": "0x4375D630687C83471829227b5C1Ea92217FD6265","quantity": {"denom": "okb","amount": "1"}}}`,
			fnCheck: func(ret *MsgWrapper, err error) {
				require.NoError(t, err)
				require.Equal(t, "grbchain/staking/MsgDeposit", ret.Name)
				require.Equal(t, "{\"delegator_address\": \"0x4375D630687C83471829227b5C1Ea92217FD6265\",\"quantity\": {\"denom\": \"okb\",\"amount\": \"1\"}}", string(ret.Data))
			},
		},
		{
			input: `{"type": "grbchain/staking/MsgWithdraw","value": {"delegator_address": "0x4375D630687C83471829227b5C1Ea92217FD6265","quantity": {"denom": "okb","amount": "1"}}}`,
			fnCheck: func(ret *MsgWrapper, err error) {
				require.NoError(t, err)
				require.Equal(t, "grbchain/staking/MsgWithdraw", ret.Name)
				require.Equal(t, "{\"delegator_address\": \"0x4375D630687C83471829227b5C1Ea92217FD6265\",\"quantity\": {\"denom\": \"okb\",\"amount\": \"1\"}}", string(ret.Data))
			},
		},
		// error
		{
			input: `{"type1": "grbchain/staking/MsgWithdraw","value":""}`,
			fnCheck: func(ret *MsgWrapper, err error) {
				require.NotNil(t, err)
			},
		},
		{
			input: `{"type": "grbchain/staking/MsgWithdraw","value1":"123"}`,
			fnCheck: func(ret *MsgWrapper, err error) {
				require.NotNil(t, err)
			},
		},
