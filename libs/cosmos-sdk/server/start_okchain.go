...
import (
	...
	sdkstoretypes "github.com/exfury/grbchain/libs/cosmos-sdk/store/types"
	storetypes "github.com/exfury/grbchain/libs/cosmos-sdk/store/types"
...
func StopCmd(ctx *Context) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				errStr := fmt.Sprintf("%s Please finish the process of grbchaind through kill -2 pid to stop gracefully", err.Error())
				cmn.Exit(errStr)
			}
			defer f.Close()
...
func nodeModeCmd(ctx *Context) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "node-mode",
		Short: "grbchaind start --node-mode help info",
		Long: `There are three node modes that can be set when the grbchaind start
